...
import { useEditableBrief } from "@/hooks/useEditableBrief";
import Editable from "@/components/preview/Editable";
...
export default function PreviewClient({ formData, brief }: PreviewClientProps) {
  ...
  const edit = useEditableBrief(formData, brief);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: t.pageBg }}>
      ...
      >
        Draft preview for <strong>{c.restaurantName}</strong> — by magical.
        <span className="hidden md:inline text-white/60"> &middot; Click any text to edit</span>
      </motion.div>
      ...
      <header className="px-6 py-6" style={{ backgroundColor: t.pageBg }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <>
              ...
              <div className="text-center">
                <Editable
                  ...
                  onChange={(v) => edit.set("restaurantName", v)}
                  as="h2"
                  className={`${t.headingFont} text-2xl md:text-3xl tracking-tight ${t.headingColor}`}
                />
                {c.tagline && (
                  <Editable
                    value={c.tagline}
                    onChange={(v) => edit.set("tagline", v)}
                    as="p"
                    className={`text-[10px] uppercase ${t.labelTracking} mt-1 font-sans ${t.mutedColor}`}
                  />
                )}
              </div>
              <nav className="hidden md:flex items-center gap-10 text-xs uppercase tracking-[0.2em] font-sans" style={{ color: t.mood === "dark" ? "#888" : "#999" }}>
                <span className="hover:opacity-70 cursor-pointer transition-opacity">Contact</span>
                <a href="#reserve" className={`px-5 py-2.5 text-white text-xs uppercase tracking-[0.15em] font-sans ${t.buttonRadius}`} style={{ backgroundColor: primary }}>Reserve</a>
              </nav>
            </>
          ) : (
            <>
              <div>
                <Editable
                  ...
                  onChange={(v) => edit.set("restaurantName", v)}
                  as="h2"
                  className={`${t.headingFont} text-2xl font-bold tracking-tight ${t.headingColor}`}
                />
                {c.tagline && (
                  <Editable
                    value={c.tagline}
                    onChange={(v) => edit.set("tagline", v)}
                    as="p"
                    className={`text-[10px] uppercase ${t.labelTracking} mt-0.5 font-sans ${t.mutedColor}`}
                  />
                )}
              </div>
          ...
          <button className={`md:hidden ${t.headingColor}`}><Menu size={22} /></button>
        </div>
      </header>

      <HeroSection content={c} edit={edit} theme={t} primaryColor={primary} location={formData.location} />
      <AboutSection content={c} edit={edit} theme={t} primaryColor={primary} />
      <MenuSection content={c} edit={edit} theme={t} primaryColor={primary} />
      <TestimonialsSection content={c} edit={edit} theme={t} primaryColor={primary} />

      {/* CTA band */}
      <section id="reserve" className={`${t.sectionPadding} px-6 text-center`} style={{ backgroundColor: t.mood === "dark" ? "#0a0a0a" : "#141414" }}>
        <p className={`text-xs uppercase ${t.labelTracking} font-sans mb-6`} style={{ color: primary }}>Reservations</p>
        <h2 className={`${t.headingFont} text-4xl md:text-5xl text-white mb-6`}>Join Us</h2>
        ...
        <a href="#reserve" className={`inline-block px-10 py-4 text-xs uppercase tracking-[0.2em] font-sans border transition-colors ${t.buttonRadius} ${t.buttonStyle === "filled" ? "text-white" : "text-white border-white/30 hover:bg-white hover:text-[#141414]"}`} style={t.buttonStyle === "filled" ? { backgroundColor: primary } : undefined}>
          Book a Table
        </a>
      </section>
      ...
      <footer className="py-20 px-6" style={{ backgroundColor: t.footerBg }}>
        <div className="max-w-4xl mx-auto text-center">
          <h3 className={`${t.headingFont} text-3xl text-white mb-3`}>{c.restaurantName}</h3>
          {c.tagline && <p className={`text-xs uppercase ${t.labelTracking} text-gray-600 font-sans mb-10`}>{c.tagline}</p>}
          ...
          <div className="border-t border-white/10 pt-8">
            <p className="text-xs text-gray-600 font-sans">&copy; 2026 {c.restaurantName} &middot; Website concept by <span style={{ color: primary }}>magical.</span></p>
          </div>
        </div>
      </footer>

      <BookingOverlay primaryColor={primary} />
      <DesignToolbar currentVibe={activeVibe} onVibeChange={setActiveVibe} primaryColor={primary} />
    </div>
  );
}
